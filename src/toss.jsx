      ...
      <div className="content">
        {!tossready && !tutorial && (
          <div className='tossing'>
            ...
            <div className="option">
              <button className='comic-button' onClick={()=>{buttonclickaudio.play(); readyhandler()}}>1 player</button>
              <button className='comic-button' onClick={()=>{buttonclickaudio.play(); readyhandler()}}>1 vs 1</button>
              <button className='comic-button' onClick={()=>{buttonclickaudio.play(); readyhandler()}}>2 vs 2</button>
            </div>
          </div>
        )}{
          tutorial  && (
            ...
            {step === 'chooseSide' && !tossDone && (
              <div className='coinchoice'>
                ...
                <div className='choicebutton'>
                  <button className='comic-button' onClick={() =>{tossaudio.play(); handleUserChoice('heads')}}>Heads</button>
                  <button className='comic-button' onClick={() => {tossaudio.play(); handleUserChoice('tails')}}>Tails</button>
                </div>
              </div>
            )}
          </div>
        ...
        {tossDone && !showVideo && result && (
          <div className='result'>
            ...
            <p>{winner}</p>
            {winner === 'You lose! Opponent wants to bat' && (<button className='comic-button' onClick={()=>{buttonclickaudio.play(); play()}}>Play</button>)}
          </div>
        )}
        {step === 'chooseAction' && !showVideo && (
          <div className="coinchoice">
            <p>CHOOSE</p>
            <div className='choicebutton'>
              <button className='comic-button' onClick={() =>{buttonclickaudio.play(0); handleActionChoice(true)}}>Bat</button>
              <button className='comic-button' onClick={() =>{buttonclickaudio.play(0); handleActionChoice(false)}}>Bowl</button>
            </div>
          </div>
        )}
      </div>
